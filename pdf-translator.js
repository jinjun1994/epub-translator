...
async function PDF(dataBuffer, options) {
    ...
    PDFJS = PDFJS ? PDFJS : require(`pdf-extraction/lib/pdf.js/${options.version}/build/pdf.js`);
    ...
    for (var i = 1; i <= counter; i++) {
        let pageText = await doc.getPage(i).then(pageData => options.pagerender(pageData,doc)).catch((err)=>{
            // todo log err using debug
            debugger;
            return "";
        });
        const {text,textContent}= pageText
        ret.text = `${ret.text}\n\n${text}`;
        ret.pageTexts.push(text)
        ret.textContents.push(textContent)
    }
